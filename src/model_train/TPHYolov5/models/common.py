...
# Thêm vào cuối file src/model_train/TPHYolov5/models/common.py



class DetectMultiBackend(nn.Module):
    # YOLOv5 MultiBackend class for inference
    def __init__(self, weights='yolov5s.pt', device=torch.device('cpu'), dnn=False, data=None, fp16=False):
        super().__init__()
        from .experimental import attempt_load

        w = str(weights[0] if isinstance(weights, list) else weights)
        pt, jit, onnx, engine, coreml, saved_model = (w.endswith(x) for x in ['.pt', '.torchscript', '.onnx', '.engine', '.mlmodel', '.pb'])
        w = attempt_load(weights if pt else w, map_location=device) if pt else w  # load PyTorch weights
        self.model = w
        self.stride = w.stride if pt else 32  # default stride
        self.names = w.names if pt else {0: 'object'}
        self.pt = pt
        self.device = device
    ...
    def forward(self, im, augment=False, profile=False):
        # Inference
        if self.pt:
            return self.model(im, augment=augment, profile=profile)
        # Các backend khác (onnx, engine,...) có thể thêm sau
